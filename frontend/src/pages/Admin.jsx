...
export default function Admin() {
  ...
  async function addNoSvc() {
    if (!boDate) return alert('날짜를 선택하세요.');
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="min-w-[900px] w-full text-sm border">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="p-2 border text-center">일반 식수</th>
                <th className="p-2 border text-center">곱빼기 식수</th>
            ...
            <tbody>
              {appRows.map((r, idx) => (
                <tr key={r.code} className="hover:bg-slate-50">
                  ...
                  <td className="p-2 border text-center">{r.base_count ?? 0}</td>
                  <td className="p-2 border text-center">{r.extra_count ?? 0}</td>
              ...
              {appRows.length === 0 && (
                <tr><td className="p-4 border text-center text-slate-500" colSpan={8}>신청자가 없습니다. 기간을 선택하고 불러오기를 눌러주세요.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="overflow-auto">
            <table className="min-w-[720px] w-full text-sm border">
              ...
              <tbody>
                ...
                {unpaidRows.length === 0 && (
                  <tr><td colSpan={6} className="p-4 text-center text-slate-500">모든 신청이 결제되었습니다.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full text-sm border">
              <thead className="bg-slate-50"><tr>
                ...
                <th className="p-2 border">부모 연락처</th>
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={5} className="p-4 text-center text-slate-500">학생이 없습니다. 엑셀 미리보기로 불러오거나 상단에서 추가해 주세요.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="card p-5"><div className="text-slate-500 text-sm">도시락 미제공일</div><div className="text-3xl font-bold mt-1">{kpi.nosvc}</div></div>
      </div>

      {policy && (
        <div className="card p-5">
          <h2 className="font-bold text-lg flex items-center gap-2"><Settings size={18} /> 전체 정책</h2>
          <div className="grid sm:grid-cols-2 gap-3 mt-3">
            <label className="text-sm">기본 가격
            ...
            <label className="text-sm">곱빼기 가격
            ...
            <div className="text-sm">이용 요일(복수 선택)
          ...
          <button
            ...
            onClick={() => {
              if (!window.confirm('경고: 모든 학생의 *선택 내역(결제 요약)* 을 초기화하시겠습니까?\n(서버의 신청 기록은 그대로 유지됩니다.)')) return;
              try {
                const LS_KEY = "doshirak.session.v1"; // 학생 신청 페이지 localStorage 키
                localStorage.removeItem(LS_KEY);
                alert("모든 학생의 선택 내역이 초기화되었습니다.\n(서버 신청 내역은 유지됨)");
