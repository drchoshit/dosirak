      ...
      {policy && (
        <div className="card p-5">
          ...
          <button
            ...
            onClick={() => {
              if (!window.confirm('⚠️ 모든 학생의 *선택 내역(결제 요약)* 을 초기화하시겠습니까?\n(서버의 신청 기록은 그대로 유지됩니다.)')) return;
              try {
                const LS_KEY = "doshirak.session.v1"; // 학생 페이지에서 쓰이는 localStorage 키
                localStorage.removeItem(LS_KEY); // 모든 학생 선택 내역(결제 요약) 삭제
                alert("✅ 전체 학생의 선택 내역이 초기화되었습니다.\n(서버 신청 내역은 유지됨)");
